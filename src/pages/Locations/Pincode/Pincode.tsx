...
import AddPincodeModal from './addPincode';
import UpdatePincodeModal from './updatePincode';
import { usePincodes,useStatusMutationPincode } from '../../../services/Api/locationApi/mutationPincode';
...
const DataTable = () => {
  ...
  const { data, refetch } = usePincodes();
  const { mutateAsync: statusMutation } = useStatusMutationPincode();
  ...
  useEffect(() => {
    if (data) {
      const updatedRows = data.map((item, index) => ({
        ...
        cityname: item.cityname.charAt(0).toUpperCase() + item.cityname.slice(1).toLowerCase(),
        pincode: item.pincode.charAt(0).toUpperCase() + item.pincode.slice(1).toLowerCase(),
  ...
  const handleStatusToggle = async (id) => {
    try {
      ...
      setSnackbarMessage(`City status updated to ${updatedStatus ? 'Active' : 'Inactive'}`);
  ...
  const columns = useMemo(() => [
    ...
    { field: 'cityname', headerName: 'City', width: 180 },
  ...
  return (
    <>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
        ...
        <AddIcon sx={{marginRight:'4px'}}/>   Add Pincode
        </Button>
      ...
      <AddPincodeModal open={addModalOpen} onClose={() => setAddModalOpen(false)} onAdd={handleAddState} />
      <UpdatePincodeModal open={updateModalOpen} onClose={() => setUpdateModalOpen(false)} state={selectedState} id={selectedState?._id} onUpdate={handleUpdateState} />
